...
void AddPercentage(CookerAudioProgressNotifier::CookingSteps* target, const int32_t kTotalCookingTimeMs,
                   double percentage) {
...
CookerAudioProgressNotifier::CookingSteps CookerAudioProgressNotifier::GetSteps(CookingType type) {
  if (!GetCookingSettings(type)) {
    qWarning() << "Invalid cooking type: " << GetCookingSettings(type).name << " (" << Underlying(type) << ")";
  }
  ...
  std::remove_if(out.begin(), out.end(), [kCookingTime](const int32_t& t) { return t >= kCookingTime || t <= 0; });

  qDebug() << GetName(type) << "Notify on:";
