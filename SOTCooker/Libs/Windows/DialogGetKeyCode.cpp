...
DialogGetKeyCode::DialogGetKeyCode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::DialogGetKeyCode)
{
    ui->setupUi(this);
    this->setModal(true);

    connect(ui->pb_unbind,&QPushButton::clicked,[&](){
        emit key(KeyVal::kUnbind,KeyVal::kUnbind);
        this->done(KeyVal::kUnbind);
    });
}

DialogGetKeyCode::~DialogGetKeyCode()
{
    delete ui;
}

void DialogGetKeyCode::keyPressEvent(QKeyEvent *event)
{
    switch (event->key())
    {
//    case Qt::Key_Escape:
    ...
    m_press_scan_code = event->nativeScanCode();
    m_press_virtual_key = event->nativeVirtualKey();
}

void DialogGetKeyCode::keyReleaseEvent(QKeyEvent *event)
{
    int scancode = event->nativeScanCode();
    int virtualkey = event->nativeVirtualKey();
    //if the key was pressed before opening the windows, ignore it
    if(scancode != m_press_scan_code || virtualkey != m_press_virtual_key){
        return;
    }
    ...
    finalvirtual = correctVirtualKey(scancode, virtualkey);

    auto returnQtKey{
        [&](int key, int scancode){
            int tempkey = virtKeyToQtKeyHash.value(key);
            int extended = scancode & EXTENDED_FLAG;
    ...
    this->done(KeyVal::kCancelled);
}


void DialogGetKeyCode::closeEvent(QCloseEvent* event)
{
    QDialog::closeEvent(event);
}

DialogGetKeyCode::KeyCode DialogGetKeyCode::GetKey(QWidget* parent)
{
    DialogGetKeyCode::KeyCode out{};
    auto dial{new DialogGetKeyCode(parent)};
    connect(dial,&DialogGetKeyCode::key,[&](int code,int scanCode){
        out.code = code;
...
int DialogGetKeyCode::GetSimpleKey(QWidget* parent){
    return GetKey(parent).code;
}
...
void DialogGetKeyCode::populateMappingHashes()
{
