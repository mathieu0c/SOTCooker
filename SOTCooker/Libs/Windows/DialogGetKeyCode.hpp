...
namespace Ui {
class DialogGetKeyCode;
}

class DialogGetKeyCode : public QDialog
{
    Q_OBJECT
...
public:
    explicit DialogGetKeyCode(QWidget *parent = nullptr);
    ~DialogGetKeyCode();

    static KeyCode GetKey(QWidget* parent);
    static int GetSimpleKey(QWidget* parent);
...
private:
    Ui::DialogGetKeyCode *ui;

    int m_press_scan_code{};
    int m_press_virtual_key{};
};
