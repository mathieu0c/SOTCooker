...
std::ostream &operator<<(std::ostream &os, const DistantVersion &dv) {
  ...
  return os << "DistantVersion{.version=" << gitversion::to_string(dv.version)
            << ", .is_preview: " << dv.is_preview << ", .assets: ["
            << assets_str.toStdString()
            << "], .installer_url: " << dv.installer_url.toStdString() << " }";
}

void RetrieveLatestDistantVersion(
    const QString &github_releases_url,
    std::function<void(std::optional<DistantVersion>)> callback,
    bool search_preview) {
  GetJsonFromAPI(
      github_releases_url, [=](std::optional<QJsonDocument> json_opt) {
        ...
        const auto kFoundIter{std::find_if(
            kVersions.rbegin(), kVersions.rend(),
            [&](const std::pair<gitversion::Version, DistantVersion> &input) {
...
std::map<gitversion::Version, DistantVersion>
ParseDistantVersion(std::optional<QJsonDocument> json_opt) {
  if (!json_opt.has_value()) {
    return {};
  }
  ...
  for (const QJsonValue &value : kVersionJson.at(0).toArray()) {
    ...
    version.version =
        gitversion::FromTag(obj["tag_name"].toString().toStdString());
